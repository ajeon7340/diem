...
export function ReportActions({ channelId, campaigns, brands=[], days=90 }: { channelId:string; campaigns:{id:string;name:string}[]; brands?:{id:string;name:string}[]; days?:number }) {
 ...
 return <div className="space-y-2.5 print:hidden">
 ...
 <details className="rounded-lg border border-line bg-paper"><summary className="cursor-pointer px-3 py-2 text-[13px] font-medium text-ink">Add to campaign</summary><form action={add} className="space-y-2 border-t border-line px-3 py-2.5"><input type="hidden" name="channelId" value={channelId}/><label className="sr-only" htmlFor="add-campaign">Campaign</label><select id="add-campaign" name="campaignId" required className="min-h-10 w-full rounded-lg border border-line bg-surface px-2.5 text-[13px]"><option value="">Choose campaign</option>{campaigns.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><div className="flex flex-wrap items-center gap-2"><Submit text="Add"/><Link className="text-[12px] text-indigo underline-offset-4 hover:underline" href={`/campaigns/new?channelId=${channelId}`}>New campaign</Link></div></form></details>

 <details className="rounded-lg border border-line bg-paper"><summary className="cursor-pointer px-3 py-2 text-[13px] font-medium text-ink">Share report</summary>
  <form action={share} className="space-y-3 border-t border-line px-3 py-2.5 text-[12px]"><input type="hidden" name="channelId" value={channelId}/>
  ...
  {shared.shareUrl && <Link href={shared.shareUrl} className="block break-all px-3 pb-2.5 text-[12px] text-indigo">{shared.shareUrl}</Link>}</details>

 <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 pt-0.5 text-[12px]">
