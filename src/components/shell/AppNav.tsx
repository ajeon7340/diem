...
const LINKS = [
  { href: '/discover', label: 'Discover creators', short: 'Discover', icon: Telescope },
  { href: '/channels', label: 'Channel analysis', short: 'Channels', icon: ChartNoAxesCombined },
  { href: '/campaigns', label: 'Campaigns', short: 'Campaigns', icon: FolderOpen },
  { href: '/settings', label: 'Settings', short: 'Settings', icon: Settings2 },
];
  ...
  return (
    <>
      ...
      <nav
        ...
        className={cn(
          'app-nav shrink-0 flex-col border-r border-line bg-surface',
          // Drawer below lg, column at lg and up.
          open ? 'fixed inset-y-0 left-0 z-50 flex w-[var(--nav-w)] shadow-[var(--shadow-overlay)]' : 'hidden',
      ...
      >
        <div className="flex h-[var(--header-h)] shrink-0 items-center gap-2 border-b border-line px-3">
          <Link
            ...
            className="flex min-w-0 items-center gap-2 text-[15px] font-semibold tracking-tight text-ink"
          >
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-[var(--r-md)] bg-indigo text-white">
          ...
          <button
            ...
            className="press ml-auto flex h-8 w-8 items-center justify-center rounded-[var(--r-md)] text-ink-muted hover:bg-paper hover:text-ink lg:hidden"
          >
            ...
            return (
              <li key={href}>
                <Link
                  ...
                  onClick={onClose}
                  className={cn(
                    'press flex min-h-9 items-center gap-2.5 rounded-[var(--r-md)] px-2.5 text-[13px] font-medium',
                ...
                >
                  {/* 1.5px beside 500-weight text: an icon carries the optical
                      weight of the label it sits next to. */}
        ...
        <div className="shrink-0 space-y-2 border-t border-line p-2">
          {workspace && !collapsed ? (
            <div className="rounded-[var(--r-md)] bg-paper px-2.5 py-2">
              <p className="truncate text-[12px] font-medium text-ink" title={workspace}>
                {workspace}
              </p>
          ...
          <div className={cn('flex items-center gap-1', collapsed && 'flex-col')}>
            <button
              ...
              className="press hidden h-8 w-8 items-center justify-center rounded-[var(--r-md)] text-ink-faint hover:bg-paper hover:text-ink lg:flex"
            >
            ...
            {signedIn ? (
              <form action="/auth/signout" method="post" className={collapsed ? '' : 'ml-auto'}>
                <button
                  type="submit"
                  title="Sign out"
                  className="press flex min-h-8 items-center rounded-[var(--r-md)] px-2 text-[12px] text-ink-faint hover:bg-paper hover:text-ink"
                >
              ...
              <Link
                href="/signin"
                className={cn(
                  'press flex min-h-8 items-center rounded-[var(--r-md)] px-2 text-[12px] text-ink-faint hover:bg-paper hover:text-ink',
                  collapsed ? '' : 'ml-auto',
                )}
              >
